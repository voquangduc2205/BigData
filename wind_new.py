...
spark = SparkSession.builder.master("local[1]").appName('WindProcess.com') \
...
wind_df = weather_df.select("station_id", "city", "obs_time_utc", "obs_time_local", "wind_gust", "wind_chill", "wind_speed", "country", "timestamp") 
wind_df.dropna()
...
def foreach_batch_function2(df, epoch_id):
    df.write.format("jdbc") \
      ...
      .option("dbtable","wind_sample").option("user","postgres") \
      .option("password", "root") \
      .mode("append").save()

wind_df.writeStream.foreachBatch(foreach_batch_function2).start().awaitTermination()
